...
import { Basurero } from "./components/Basurero";
  ...
  return (
    <div className="App">
      <div className="w3-row" style={{display:"flex", flexDirection:"row"}}>
        <ul style={{backgroundColor:"#A7A59A", padding:"2%", margin:"20px",  borderRadius:"10px", width:"20%"}}>
        <h1>Menú</h1>
          {enlistar().map((item, index) => (
            <li key={index}>
              <button type="button" onClick={() => actualizar(item)} style={{marginRight:"10px"}}>{item}</button>
